...
def choose_search() -> str:
    q = input("Enter YouTube URL or keywords: ").strip()
    if is_url(q):
        return q
...
def maybe_trim(mp3_path: str) -> str:
    if not confirm("Trim audio? "):
        return mp3_path
    mode = input("Trim mode: [1] Manual times  [2] Interactive waveform  (Enter 1/2): ").strip() or "1"
    if mode == "1":
...
def maybe_metadata(mp3_path: str) -> None:
...
def maybe_trim(mp3_path: str) -> str:
    ...
    if mode == "1":
        try:
            raw_start = input("Start (s or mm:ss, blank=0): ").strip()
            start = parse_time_input(raw_start)
            raw_end = input("End (s or mm:ss, blank=end): ").strip()
...
def maybe_cover(mp3_path: str, url: str) -> None:
    ...
    if mode == "1":
        try:
            raw_ts = input("Timestamp (s or mm:ss): ").strip()
